...
app.get('/upay', (req, res) => {
    if (!req.query.order) {
        ...
        //查询订单号分配的收款地址、金额、订单过期时间、订单完成后跳转页面
            ...
            <div class="content">
                <div class="section">
                    <div class="title">
                        ...
                        <h1 class="amount parse-action" id="payment_r" data-ref="#parse-amount" style="font-size: 40px; margin-top: -10px;margin-bottom: 0px;" data-msg="金额已复制">
                            `+ amount + `
                        </h1>
                    ...
                    <div class="address parse-action" data-ref="#parse-address" data-msg="地址已复制">
                    `+ address + `
                    </div>
                    <div class="main">
                        <img class="qrcode" src="/`+ address + `.png" alt="qrcode">
                    </div>
        ...
        !function(a) {
            a.fn.downCount = function(b, c) {
                ...
                return this.reset = function() {
                    clearInterval(h),
                    f.find(".days").text("00"),
                    f.find(".hours").text("00"),
                    f.find(".minutes").text("00"),
                    f.find(".seconds").text("00")
                }
                ,
                this
            }
        ...
        function() {
            ...
            var a = function() {
                var a = window.location.origin
                  , b = 10000
                  , c = {
                    transactionQuery: a + "/paystatus"
                }
                  , d = null
                  , e = null;
                return {
                    api: c,
                    transactionQueryInterval: b,
                    transactionQuery: d,
                    downCounter: e
                }
            }()
...
app.get('/paystatus', (req, res) => {
    ...
    //支付部分
    //res.json({'code':401,'balance':'1.00','remaining':'200'})
    res.json({'code':1})
})
